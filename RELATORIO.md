...
| *(atual)* | **Salvar lugares, selo de destaque e compartilhar localização (SW v19, 35 testes):** ① botão **🔖 Salvar** nos lugares (tabela `saved_spots`, `POST /api/spots/:id/save`, aba **Salvos** no perfil); ② **selo ⭐ Destaque** para lugares populares (≥ 5 curtidas) no popup e no feed (`featured`); ③ **link de localização** (`#loc=lat,lng`) gerado pelo botão **📍 Compartilhar** (share/clipboard) e tratado no deep link; ④ **empty states** guiados no feed (buscar pessoas / nova foto / explorar região); ⑤ ajustes visuais finos no tema claro (popup, zoom, markers). Validado via CDP. |
---
